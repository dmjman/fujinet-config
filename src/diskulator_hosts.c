...
void diskulator_hosts_keys_clear_bottom_rows(void)
{
...
{
	diskulator_hosts_keys_clear_bottom_rows();
	screen_puts(0,20,
		CH_KEY_1TO8 "Slot"
...
void diskulator_hosts_keys_devices(void)
{
	...
	screen_puts(3,20,
		...
		CH_KEY_LABEL_L CH_INV_C CH_INV_L CH_INV_E CH_INV_A CH_INV_R CH_KEY_LABEL_R "All"
		CH_KEY_LABEL_L CH_INV_S CH_KEY_LABEL_R "witch");
	screen_puts(3,21,
		CH_KEY_TAB "Hosts"
...
{
	char         new;
	char         oldTemp[2]   = { old + '1', 0 };
	char         newTemp[2]   = { 0, 0 };
	char * const switchPrompt = "To device: ";

	// Get slot
	
	// Write to bottom rows
	diskulator_hosts_keys_clear_bottom_rows();
	screen_puts(0, 20, switchPrompt);

	// Get input from user and assign to new slot variable
	screen_input(11, 20, &new);
	newTemp[0] = new + '1';

	// Rewrite original text
	diskulator_hosts_keys_devices();

	// Mount the new slot with the current host slot
	fuji_sio_mount_device(new, context->deviceSlots.slot[old].mode);
	screen_clear_line(new + pos - 2);
	screen_puts(2, new + pos - 2, newTemp);
	screen_puts(5, new + pos - 2, context->deviceSlots.slot[new].file);

	// Unmount the original slot
	fuji_sio_umount_device(old);
	screen_clear_line(old + pos - 2);
	screen_puts(2, old + pos - 2, oldTemp);
	screen_puts(5, old + pos - 2, text_empty)
}
	...
	while (*new_substate==DEVICES)
		{
			...
			switch(k)
				...
				case 'R':
					...
					break;
				case 'S':
					diskulator_hosts_switch_device_slot(i, ORIGIN_DEVICE_SLOTS, context);
					break;
				case 'W':
